...
- `fiscal_index`: 財政力指数。SSDSに2点以上の系列がある場合のみ保持する任意指標（特別区など、SSDS側で市区町村財政指標がそろわない地域では省略可）。
...
- `fiscal_index` は都道府県が `D2101_財政力指数（都道府県財政）`、市区町村が `D2201_財政力指数（市町村財政）`。取得できない地域では `indicators` と `source.statsDataIds` の両方から省略する。
...
- 任意指標は、対象範囲内で取得できる直近最大10点を採用する。選挙年のみ値がある指標は、年が連続しなくてよい。
